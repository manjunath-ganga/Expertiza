...
module DynamicReviewMapping   
  ...
  def assign_individual_reviewer(round_num)
    ...
    for i in 0 .. reviewers.size - 1
      ...
      for j in 0 .. authors.size - 1
        current_author_candidate = (current_author_candidate + stride) % authors.size
        if current_author_candidate !=  current_reviewer_candidate
 ...
 def assign_reviewers_for_team(round_num)
   ...
   if teams.size == 0
     raise "No teams available for assignment"
   end
   for i in 0 .. reviewers.size - 1
     current_reviewer_candidate = i
     current_team_candidate = current_reviewer_candidate
     for j in 0 .. teams.size - 1
          current_team_candidate = (current_team_candidate + stride) % teams.size
          if !isMemberOfTeam(reviewers[current_reviewer_candidate], teams[current_team_candidate])
            TeamReviewResponseMap.create(:reviewee_id => teams[current_team_candidate].id, :reviewer_id => reviewers[i].id, :reviewed_object_id => self.id)
          end
       end
   end
   end

  def isMemberOfTeam(member, team)
        TeamsUser.exists?({"team_id"=>team.id, "user_id"=>member.user_id})
   end
